...
| [INV-17](cards/INV-17-historico-corrompido-derruba-o-download.md) | **Um registro corrompido no histórico derruba o download da invoice** — `TypeError: Cannot read properties of undefined (reading 'trim')`. Achado pela suíte do QA-04 (01/09), escrevendo `invoice/history.spec.ts`. O filtro do `ler()` aceita `{draft: {}}` porque só exige `typeof draft === 'object'`, e a `assinatura()` faz `d.invoiceNumber.trim()` na primeira linha. Como `recordDownload()` assina **todo** registro guardado para achar a duplicata, um registro velho e ruim impede o download de uma invoice nova e válida — e o download é o único desfecho da tela. Causa de fundo: o `storage.ts` descarta o que não é da versão atual (`version !== DRAFT_VERSION`), e o `history.ts` **não tem checagem de versão nenhuma**, apesar de ler a mesma chave `.v1` fixa. **O teste já está no repositório** — mas o **Vitest 4 removeu o `it.failing`** que o QA-02 usou no Jest (`TypeError: it.failing is not a function`, medido), então ele afirma o comportamento errado à mão e se chama `BUG INV-17 · ... AINDA derruba o download`: verde hoje, falha no dia em que alguém corrigir sem apagá-lo — confirmado aplicando a correção | P | `frontend/src/invoice/history.ts` |
...
| --- | --- | --- | --- |
| [QA-04](cards/QA-04-testes-de-listavagas.md) | **Testes de componente para `ListaVagas`** | M | Três bugs medidos e sem teste: perda de escolha durante o upload do CV, o selo `CV` mentindo depois de "Clear filters", a contagem divergindo dos selos. Ficou de fora da camada 4 do QA-03 por custo — 1.053 linhas, e testá-la mal produziria a cobertura sem teste real que aquele card existe para não fazer. **A infraestrutura já está pronta**: jsdom, Testing Library, `setupFiles` com `cleanup()`, `*.spec.tsx` no `include` |
