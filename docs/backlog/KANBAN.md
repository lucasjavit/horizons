...
| [PLT-11](cards/PLT-11-gestao-de-usuarios.md) | **A tela onde o dono gerencia os usuários** — lista, papel, ativo/inativo | M | pedido em 31/08 — é o que torna o `MANAGER` do PLT-09 real. **A armadilha:** o papel é recalculado a cada login, então promover pela tela sem mexer no `auth.service.ts` seria desfeito em silêncio |
